...
local function transition_item(new_status)
	...
	if new_status == "open" then
		target = find_item(items, rel_file, line, function(s)
			return s ~= "open"
		end)
	else
		target = find_item(items, rel_file, line, function(s)
...
local function command_pr(opts)
  local base_ref = opts and opts.args and opts.args ~= "" and opts.args or nil
  pr_view.open(base_ref)
end
...
function M.register()
	...
	vim.api.nvim_create_user_command("ReviewPR", command_pr, { nargs = "?" })
